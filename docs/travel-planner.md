...
6. **Payment (optional)** starts collapsed for new arrangements. Expand it to enter an amount/currency and mark it **Paid**, which creates one linked expense. Existing amounts and paid records open expanded. Confirmation/flight/train numbers are visible in the main booking form. Other status and technical details remain in **More details**.
7. In **Spending**, add an ordinary purchase directly and choose its destination without making an itinerary item. New purchases inherit the current destination and default the payment date to today. Opening a linked expense edits its original arrangement; ordinary expenses have their own editor. Optional **Add booking details** and **Add to itinerary** reuse an existing expense's identity, payment and destination.
...
Forms show essential fields first. Place notes and links and booking references stay visible; **More details** holds status and secondary fields. Payment has its own optional section. Map actions and edit controls are in item details, keeping the daily itinerary compact.
...
Drift schema 3 added `trip_plans`, keyed by local trip ID. Schema 4 adds stable expense identities and itinerary item references. Schema 5 adds the optional standalone expense destination. Plan synchronization runs before expense queue upload so newly created destinations exist before their purchases are sent. Documents use stable random item IDs, item references, ordered arrays and link lists. The planning document is authoritative for payment amounts and status; existing ledger names and notes are preserved when shared itinerary text changes; the expense row is updated in the same local/server transaction. Writes carry a new mutation ID. Pending documents and their payments survive restarts and failures.
...
`GET/PUT /expenses/api/travel/trips/{id}/plan/` includes `payment_ids` (expense client identity to server ID). Booking/activity payment fields are `amount`, `currency`, `paymentStatus`, `paidDate`, `expenseClientId` and `expenseCategory`. Existing travel expense endpoints include `client_id`, nullable `plan_item_id`, and optional `destination_id`. Omitted destination fields on updates preserve the existing assignment for older clients; explicit empty values clear it. Destination IDs must belong to the trip. Removing destinations clears assignments without deleting expenses. Creating with the same `client_id` returns the existing record without duplicating it. The plan revision protects linked expense edits as well as itinerary edits.
